...
import { DocumentData, Expense } from '../types';
...
export const Preview: React.FC<PreviewProps> = ({ data }) => {
   ...
   */
  const chunkExpenses = () => {
    const USABLE_HEIGHT_MM = 245; // Safety buffer
    ...
    data.expenses.forEach((expense, index) => {
      ...
      const spaceNeeded = itemHeight + (isLast ? FOOTER_HEIGHT : 0) + GAP_HEIGHT;

      if (currentPageHeight + spaceNeeded > USABLE_HEIGHT_MM && pages[pages.length - 1].length > 0) {
        pages.push([expense]);
            ...
            {pageIdx === 0 && (
              <div className="mb-12">
                <div className="flex justify-between items-baseline font-bold text-3xl mb-8">
                  <span className="underline decoration-black decoration-2 underline-offset-[14px]">{data.headerTitle}</span>
                  <span className="underline decoration-black decoration-2 underline-offset-[14px]">{data.name}</span>
                  <span className="underline decoration-black decoration-2 underline-offset-[14px]">{formatDate(data.date)}</span>
                </div>
                <div className="text-center font-bold text-2xl py-4 border-y-2 border-slate-100">
                  Adv - {data.advanceAmount.toLocaleString()}/-
                </div>
              </div>
            )}
            ...
            <div className={`flex-1 ${pageIdx > 0 ? 'pt-4' : ''}`}>
              <div className="flex flex-col gap-6">
                {pageExpenses.map((expense) => {
                  const globalIdx = data.expenses.findIndex(e => e.id === expense.id);
                  return (
                      ...
                      {expense.type === 'job' ? (
                        <div className="space-y-2">
                          <div className="flex justify-between items-baseline font-bold text-xl leading-tight">
                            <div className="flex-1 flex items-baseline gap-2">
                              <span className="flex-shrink-0 min-w-[20px]">{globalIdx + 1}.</span>
                              <div className="flex-1">
                                <span dangerouslySetInnerHTML={{ __html: expense.title }} />
                              </div>
                            </div>
                            <div className="amount-col tabular-nums text-right">{expense.baseAmount.toLocaleString()}</div>
                          </div>
                          {expense.subExpenses.map((sub) => (
                            <div key={sub.id} className="flex justify-between items-baseline ml-7 text-lg">
                              <div className="flex-1 myanmar-font text-slate-800">{sub.label}</div>
                              <div className="amount-col tabular-nums text-slate-800 text-right">{sub.amount.toLocaleString()}</div>
                            </div>
                          ))}
                        </div>
                      ) : (
                        <div className="flex justify-between items-baseline font-bold text-xl leading-tight">
                          <div className="flex-1 flex items-baseline gap-2">
                            <span className="flex-shrink-0 min-w-[20px]">{globalIdx + 1}.</span>
                            <div className="flex-1">
                              <span className="myanmar-font" dangerouslySetInnerHTML={{ __html: expense.label }} />
                              {expense.date && (
                          ...
                          <div className="amount-col tabular-nums text-right">{expense.amount.toLocaleString()}</div>
                        </div>
                      )}
                    </div>
                  );
            ...
            {pageIdx === pages.length - 1 && (
              <div className="mt-auto pt-10 border-t-2 border-slate-100 flex justify-end">
                <div className="w-full max-w-[340px]">
                  ...
                  <div className="flex justify-between items-baseline py-2 text-xl font-semibold text-slate-500">
                    <span className="text-sm uppercase tracking-widest">Adv</span>
                    <span className="tabular-nums">{data.advanceAmount.toLocaleString()}/-</span>
                  </div>
                  <div className="border-t-[5px] border-double border-black mt-4 pt-5 flex justify-between items-center">
                    <span className="font-bold text-3xl myanmar-font leading-none">{data.balanceLabel}</span>
